...
int main(int argc, char **argv) {
    ...
    Eigen::Vector3d hololens_right_wrist_pose = Eigen::Vector3d::Zero();
    Eigen::Vector3d previous_hololens_right_wrist_pose = Eigen::Vector3d::Zero();
    Eigen::Quaterniond hololens_right_wrist_rot = Eigen::Quaterniond(1, 0, 0, 0);
    ros::Subscriber
    sub_azure = node_handle.subscribe<geometry_msgs::Transform>(
            "/right/goal_wrist_transform_stamp", 1,
            boost::function < void(
    const geometry_msgs::Transform &)>(
             [&](const geometry_msgs::Transform &data) {
                std::lock_guard <std::mutex> lock(hand_transform_mutex);
                  hololens_right_wrist_pose.x() = data.translation.x;
                  hololens_right_wrist_pose.y() = data.translation.y;
                  hololens_right_wrist_pose.z() = data.translation.z;
                  hololens_right_wrist_rot.x() = data.rotation.x;
                  hololens_right_wrist_rot.y() = data.rotation.y;
                  hololens_right_wrist_rot.z() = data.rotation.z;
                  hololens_right_wrist_rot.w() = data.rotation.w;
            }));
      ...
      // Wait for next control cycle
       ...
       if (!timing_ok) {
        ...
        Eigen::Matrix3d m = Eigen::Matrix3d::Zero();
        {
            ...
            hololens_right_wrist.translation() = hololens_right_wrist_pose;
            hololens_right_wrist.linear() = hololens_right_wrist_rot.toRotationMatrix();
