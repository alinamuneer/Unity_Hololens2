...
namespace ArUcoDetectionHoloLensUnity
{
    ...
    public class ArUcoMarkerDetection : MonoBehaviour
    {
        ...
        /// </summary>
        private GameObject tagLink;
        private GameObject tagBase;
        public GameObject markerGo;
        private int detectionCount = 0;

        public Dictionary<int, GameObject> detectedTags;
        ...
        async void Start()
        {
            ...
            detectedTags = new Dictionary<int, GameObject>();

            tagLink = new GameObject();
            tagLink.name = "tag_" + Id;
            
            tagBase = new GameObject();
            tagBase.name = "tagBase";

            tagLink.transform.SetParent(tagBase.transform);
            tagLink.transform.position = Vector3.zero;
            tagLink.transform.rotation = Quaternion.identity;
            tagLink.transform.Rotate(new Vector3(0, 90, 90), Space.Self);
            markerGo.transform.SetParent(tagBase.transform);
        ...
        async void Update()
        {
            ...
            if (_frameCount == skipFrames)
            {
                ...
                {
                    GameObject.Find("Script Holder").SetActive(false);
                    GameObject.Find("AsyncCoroutineRunner").SetActive(false);
                }
            }
#endif
        }
        ...
        void UpdateArUcoDetections(IList<DetectedArUcoMarker> detections)
        {
            ...
            if (detections.Count != 0)
            {
                ...
                if (_isWorldAnchored)
                {
                    try
                    {
                        DestroyImmediate(tagBase.GetComponent<WorldAnchor>());
                        //DestroyImmediate(markerGo.GetComponent<WorldAnchor>());
                        //DestroyImmediate(tagLink.GetComponent<WorldAnchor>());
                ...
                {
                    /*if (!detectedTags.ContainsKey(detectedMarker.Id))
                    {
                        detectedTags.Add(detectedMarker.Id, Instantiate(markerGo, new Vector3(0, 0, 0), Quaternion.identity));
                        detectedTags[detectedMarker.Id].transform.localScale = new Vector3(markerSize, markerSize, markerSize);
                    }*/

                    if (detectedMarker.Id != Id)
                        continue;
                    ...
                    //tagLink.transform.SetPositionAndRotation(
                    //    CvUtils.GetVectorFromMatrix(transformUnityWorld),
                    //    CvUtils.GetQuatFromMatrix(transformUnityWorld));

                    tagBase.transform.SetPositionAndRotation(
                        CvUtils.GetVectorFromMatrix(transformUnityWorld),
                        CvUtils.GetQuatFromMatrix(transformUnityWorld));

                    // fit apriltag from ros
                    //tagLink.transform.Rotate(new Vector3(0, 90, 90), Space.Self);

                    //tagRos.transform.SetPositionAndRotation(tagLink.transform.position, RosSharp.TransformExtensions.Ros2Unity(tagLink.transform.rotation));
                    detectionCount++;
                }
            }
            ...
            else
            {
                ...
                //tagLink.AddComponent<WorldAnchor>();
                tagBase.AddComponent<WorldAnchor>();
